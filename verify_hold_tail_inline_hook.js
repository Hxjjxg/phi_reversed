📦
146561 /verify_hold_tail_inline_hook.js
75436 /verify_hold_tail_inline_hook.js.map
✄
...
// verify_hold_tail_inline_hook.ts
var require_verify_hold_tail_inline_hook = __commonJS({
  "verify_hold_tail_inline_hook.ts"(exports) {
    init_node_globals();
    Object.defineProperty(exports, "__esModule", { value: true });
    init_dist();
    var CREATE_NOTE_HOLD_BODY_HL_WRITE_OFFSET = 3016;
    var MAX_INLINE_LOGS = 120;
    var MAX_NOTEMOVE_LOGS = 120;
    Il2Cpp.perform(() => {
      ...
      function readFloat(base, offset) {
        if (base.isNull())
          return Number.NaN;
        try {
      ...
      function arrayLen(arrayPtr) {
        return readS32(arrayPtr, 24);
      }
      function arrayItem(arrayPtr, index) {
        const len = arrayLen(arrayPtr);
        if (arrayPtr.isNull() || index < 0 || index >= len)
          return NULL;
        return readPtr(arrayPtr, 32 + index * Process.pointerSize);
      }
      ...
      function formatRealTime(notePtr) {
        const realTime = readFloat(notePtr, 44);
        return Number.isFinite(realTime) ? realTime.toFixed(6) : "?";
      }
      const inlineHitsByHold = /* @__PURE__ */ new Map();
      const seenNoteMove = /* @__PURE__ */ new Set();
      ...
      Interceptor.attach(hookAddr, {
        onEnter() {
          const judgeLine = ptrOf(this.context.x19);
          const holdControl = ptrOf(this.context.x22);
          const noteImages = ptrOf(this.context.x23);
          const x20 = ptrOf(this.context.x20);
          const holdKey = hex(holdControl);
          const fieldNoteImages = readPtr(holdControl, 72);
          const fieldJudgeLine = readPtr(holdControl, 56);
          const noteInfo = readPtr(holdControl, 48);
          const holdHl0 = readPtr(judgeLine, 64);
          const holdHl1 = readPtr(judgeLine, 72);
          const chordSupport = readBool(judgeLine, 268);
          ...
          if (inlineLogCount < MAX_INLINE_LOGS) {
            inlineLogCount++;
            console.log(`[hold-inline #${inlineLogCount}] hold=${holdKey} noteImages=${hex(noteImages)} len=${len} t=${formatRealTime(noteInfo)} chord=${chordSupport}
  ...
  before-store: arr[0]=${hex(head0)} HoldHL0=${hex(holdHl0)} arr0==HL0:${headAlreadyHl0}
      ...
      Interceptor.attach(noteMove.virtualAddress, {
        onEnter(args) {
          ...
          if (seenNoteMove.has(holdKey))
            return;
          seenNoteMove.add(holdKey);
          const inlineInfo = inlineHitsByHold.get(holdKey);
          if (!inlineInfo)
            return;
          const noteImages = readPtr(hold, 72);
          const noteInfo = readPtr(hold, 48);
          const head0 = arrayItem(noteImages, 0);
          const body1 = arrayItem(noteImages, 1);
          const tail2 = arrayItem(noteImages, 2);
          ...
          if (noteMoveLogCount < MAX_NOTEMOVE_LOGS) {
            noteMoveLogCount++;
            console.log(`[hold-inline/notemove #${noteMoveLogCount}] hold=${holdKey} t=${formatRealTime(noteInfo)} inlineT=${inlineInfo.realTime}
