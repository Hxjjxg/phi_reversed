...
// hook_hold_png.ts
var require_hook_hold_png = __commonJS({
  "hook_hold_png.ts"(exports) {
    init_node_globals();
    Object.defineProperty(exports, "__esModule", { value: true });
    init_dist();
    var PNG_DIR = "/data/local/tmp";
    var FILES = {
    ...
    Il2Cpp.perform(() => {
      ...
      const spriteCreate = Sprite.method("Create", 3);
      const loadImage = ImageConversion.method("LoadImage", 2);
      function readBytes(path) {
        try {
          const f = new File(path, "rb");
          const buf = f.readBytes();
          f.close();
          const u8 = new Uint8Array(buf);
          const arr = Il2Cpp.array(Byte, u8.length);
      ...
      function buildSprite(file) {
        ...
        const w = tex.method("get_width").invoke();
        const h = tex.method("get_height").invoke();
      ...
      for (const [slot, file] of Object.entries(FILES))
        sprites[slot] = buildSprite(file);
      globalThis.__holdSprites = sprites;
      ...
      Interceptor.attach(start.virtualAddress, {
        onEnter(args) {
          ...
          const arr = self.field("noteImages").value;
          if (arr.isNull() || arr.length < 3)
            return;
          const judgeLine = self.field("judgeLine").value;
          let isHL = false;
          if (!judgeLine.isNull()) {
            const hl1 = judgeLine.field("HoldHL1").value;
            const head2 = arr.get(1);
            isHL = !head2.isNull() && !hl1.isNull() && head2.handle.equals(hl1.handle);
          }
