    ...
    Il2Cpp.perform(() => {
      ...
      const ensureLoadedSprites = (levelControl) => {
        ...
        if (loadedSprites.multi.holdEnd) {
          multiHoldEndPtr = loadedSprites.multi.holdEnd.handle;
        }
        return loadedSprites;
      };
      if (Number(HOLD_TAIL_MODE) === HOLD_TAIL_MODE_SEPARATE) {
        const base = Il2Cpp.module.base;
        Interceptor.attach(base.add(37321656), {
          onEnter() {
            if (multiHoldEndPtr === null || multiHoldEndPtr.isNull())
              return;
            const noteImages = this.context.x22;
            if (noteImages.isNull())
              return;
            const len = noteImages.add(24).readU32();
            if (len >= 3) {
              noteImages.add(48).writePointer(multiHoldEndPtr);
            }
          }
        });
        Interceptor.attach(base.add(37322596), {
          onEnter() {
            if (multiHoldEndPtr === null || multiHoldEndPtr.isNull())
              return;
            const noteImages = this.context.x23;
            if (noteImages.isNull())
              return;
            const len = noteImages.add(24).readU32();
            if (len >= 3) {
        ...
        console.log("[note-texture] inline hooks installed at CreateNote for multi hold tail (SEPARATE mode)");
      }
