...
function createCustomSprite(imagePath: string, pixelsPerUnit: number = 100): any {
    ...
    const bytes = readLocalBytes(imagePath);
    const systemByte = Il2Cpp.corlib.class("System.Byte");
    const byteArray = Il2Cpp.array(systemByte, bytes);
...
Il2Cpp.perform(() => {
    (globalThis as any).createCustomSprite = createCustomSprite;
    console.log("[sprite.ts] ready: globalThis.createCustomSprite(path, pixelsPerUnit?)");
});