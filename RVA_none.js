📦
155576 /scripts/local_saves/cloud_sync_version_bypass_bridge.js
✄
...
var require_cloud_sync_version_bypass_bridge = __commonJS({
  "scripts/local_saves/cloud_sync_version_bypass_bridge.ts"(exports) {
    ...
    function installSummarySourceBypassHook(assemblyImage) {
      ...
      let pendingLocalSummary = null;
      let pendingLocalSummaryAt = 0;
      fromLocalSaveMethod.implementation = function() {
        ...
        try {
          ...
          console.log(`[cloud-sync] FromLocalSave captured local(gv=${formatMaybeNumber(local.gameVersion)},sv=${formatMaybeNumber(local.saveVersion)})`);
        } catch {
        }
        return localSummary;
      };
      deserializeMethod.implementation = function(base64) {
        const cloudSummary = this.method("Deserialize", 1).invoke(base64);
        try {
          const cloud = readSummaryVersions(cloudSummary);
          const isRecentPair = pendingLocalSummary && Date.now() - pendingLocalSummaryAt <= 15e3;
          if (isRecentPair) {
            ...
            if (shouldBypassSave || shouldBypassGame) {
              ...
              console.log(`[cloud-sync] source bypass hit local(gv=${formatMaybeNumber(local.gameVersion)},sv=${formatMaybeNumber(local.saveVersion)}) cloud(gv=${formatMaybeNumber(cloud.gameVersion)},sv=${formatMaybeNumber(cloud.saveVersion)}) patchedLocal(gv=${formatMaybeNumber(localAfter.gameVersion)},sv=${formatMaybeNumber(localAfter.saveVersion)})`);
            }
          }
        } catch (e) {
...
export default require_cloud_sync_version_bypass_bridge();
