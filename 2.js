...
var require_note_texture_replace_bridge_changed = __commonJS({
  "scripts/note_texture_hook/note_texture_replace_bridge changed.ts"(exports) {
    ...
    var HOLD_TAIL_MODE = HOLD_TAIL_MODE_SHARED;
    var NOTE_TEXTURES = {
      normal: {
    ...
    function applyHoldNoteImages(noteImages, sprites) {
      ...
      noteImages.set(2, sprites.holdEnd ?? null);
    }
    ...
    function syncMultiHoldTailOnce(instance, sprites) {
      ...
      const isMulti = sameObject(noteImages.get(0), sprites.multi.holdHead) || sameObject(noteImages.get(1), sprites.multi.holdBody);
      if (!isMulti) {
        return;
      }
      noteImages.set(2, sprites.multi.holdEnd ?? null);
