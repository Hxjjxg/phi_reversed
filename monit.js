...
`;
      ...
      if (recent.length > 0) {
        ...
        for (const entry of recent) {
          const dt = (entry.deltaT * 1e3).toFixed(1);
...
`;
    ...
    Il2Cpp.perform(() => {
      ...
      ScoreControl.method("Perfect").implementation = function(...args) {
        const noteCode = args[0];
        const judgeTime = args[1];
        stats.perfect++;
        stats.totalNotes++;
        stats.deltaTList.push({ noteCode, deltaT: judgeTime, judge: "Perfect" });
        return this.method("Perfect").invoke(...args);
      };
      ScoreControl.method("Good").implementation = function(...args) {
        const noteCode = args[0];
        const judgeTime = args[1];
        if (judgeTime <= 0) {
          stats.goodEarly++;
      ...
      ScoreControl.method("Bad").implementation = function(...args) {
        const noteCode = args[0];
        const judgeTime = args[1];
        stats.bad++;
        stats.totalNotes++;
      ...
      ScoreControl.method("Miss").implementation = function(...args) {
        const noteCode = args[0];
        stats.miss++;
        stats.totalNotes++;
      ...
      ScoreControl.method("Update").implementation = function(...args) {
        const result = this.method("Update").invoke(...args);
        try {
          const comboTextPtr = this.handle.add(48).readPointer();
          ...
          try {
            const comboText = this.field("comboText").value;
      ...
      ScoreControl.method("GetLevelResultInfo").implementation = function(...args) {
        const result = this.method("GetLevelResultInfo").invoke(...args);
        const score = result.field("score").value;
        const percent = result.field("percent").value;
        const perfect = result.field("perfect").value;
        const good = result.field("good").value;
        const bad = result.field("bad").value;
        const miss = result.field("miss").value;
        const early = result.field("early").value;
        const late = result.field("late").value;
        const maxCombo = result.field("maxCombo").value;
        try {
          const comboTextPtr = this.handle.add(48).readPointer();
          if (!comboTextPtr.isNull()) {
            const comboText = new Il2Cpp.Object(comboTextPtr);
